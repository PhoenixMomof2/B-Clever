...
import { deleteAllowance } from '../redux/action/allowanceAction';

const AllowanceCard = ({allowance, currentKid}) => {   
  const dispatch = useDispatch()
  
  const handleDeleteClick = () => {
    dispatch(deleteAllowance(allowance.id))
  }
    ...
    return (
      <div className="bg-white overflow-hidden shadow-xl rounded-2xl ">       
        ...
        <div key={allowance.id} className="p-6">
          <h1 className="font-bold text-blue-500 text-3xl border-b">{currentKid.name} earned a ${allowance.balance} allowance!</h1>
          ...
          <div className="flex gap-4 py-2">
            <Link className="px-3 bg-purple-500 rounded border-white border-3" state={{ allowance }} to={`/allowances/${allowance.id}/edit`}>Edit</Link>
